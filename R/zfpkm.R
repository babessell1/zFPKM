...
zFPKMCalc <- function(fpkm, min_thresh) {
  ...
  local_maxes <- find_maxima(d[["roll_y"]][floor(0.3*length(d[["roll_y"]])):ceiling(0.8*length(local_maxesd[["roll_y"]]))])
  print(local_maxes)
  ...
  while  ( FALSE && (max_y < 0.1*max(d[["y"]])) && (cnt < 20) ) { # while selected local max y is less than 20% of actual maximum 
	cnt <- cnt + 1
	...
	#local_maxes <- local_maxes[floor(0.3*length(local_maxes)):ceiling(0.8*length(local_maxes))]
    fit_max <- max(local_maxes) + as.integer(perc/2)
    # Set the maximum point in the density as the mean for the fitted Gaussian
